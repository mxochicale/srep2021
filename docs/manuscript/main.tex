...
\includegraphics[width=1.0\textwidth]{figures/fig01/versions/drawing-v01}
	...
	{\bf State space reconstruction methodology (A),
		Minimum delay embedding values with AMI's method and minimum dimension embedding values with Cao's method (B), and
		Recurrence Plots and its patterns (C).
	}
...
(Fig~\ref{fig01}b1), and a Gaussian noise time series with zero mean
and a variance of one (Fig~\ref{fig01}b5).
...
are within the threshold of $1\pm0.05$ (Fig~\ref{fig01}b2).
$E_2(m)$ values, for chaotic time series, are different to one 
(Fig~\ref{fig01}b3), for which, it can be concluded that for the
...
which is defined by the threshold of $1\pm0.05$ (Fig~\ref{fig01}b6).
...
approximately equal to one (Figure~\ref{fig01}b7) for which it can be
...
standard deviation equal to one. From Fig~\ref{fig01}(b4, b8), it can then be
...
(Fig~\ref{fig01}(c1,c2)) \cite{eckmann1987, marwan2007,marwan2015}.
...
uniformly distributed noise (Fig~\ref{fig01}c3),
...
(Fig~\ref{fig01}c7) \cite{eckmann1987, marwan2015}.
...
(Figure~\ref{fig02}a1,a3). Data were then collected with four NeMEMSi
...
(i) ten repetitions of horizontal arm movement at normal (HN) 
	and faster (HF) speed (Figure~\ref{fig02}a1), and
(ii) ten repetitions of vertical arm movement at normal (VN) 
	and faster (VF) speed (Figure~\ref{fig02}a3).
...
and vertical arm movements, Fig~\ref{fig02}B shows smoothed time series
...
arm movements (axis GyroZ) and vertical arm movements (axis GyroY) (Figs \ref{fig02}C).
%%%---------------------------------(FIGURE)-------------------------------------
%\begin{figure}[ht]
%\centering
...
are shown for the reconstructed state spaces of horizontal (figures~\ref{fig04}(A))
and vertical (figures~\ref{fig04}(B)) arm movements.
...
\caption{
	...
	RQA metrics for (c1) REC, (c2) DET, (c3) RATIO, and (c4) ENTR of
...
%\subsubsection*{REC values}
Figure~\ref{fig05}(c1) presents box plots of REC values,
...
%\subsubsection*{DET values}
Figure \ref{fig05}(c2) presents DET values and shows
...
Figure \ref{fig06}(A) shows that increasing recurrence
...
for both human (HS01) and (RS01) (Figures~\ref{fig06}(B)).
