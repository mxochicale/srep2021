...
##VERSION 
version <- '00'
feature_path <- '/docs/figures/caoami/src'
...
MTD$axis <- gsub('sg0zmuvGyroZ', 'sg0', MTD$axis) 
MTD$axis <- gsub('sg1zmuvGyroZ', 'sg1', MTD$axis) 
MTD$axis <- gsub('sg2zmuvGyroZ', 'sg2', MTD$axis) 
...
pRS01HNMTD <- ggplot(RS01HNMTD, aes(x=axis, y=timedelays) )+
          geom_point(aes(fill=axis),
                    alpha=0.8,
                    size=4,
                    shape=21,
...
pRS01HFMTD <- ggplot(RS01HFMTD, aes(x=axis, y=timedelays) )+
          geom_point(aes(fill=axis),
                    alpha=0.8,
                    size=4,
                    shape=21,
