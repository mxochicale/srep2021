...
#
		...
		# Substitute same values 
		#set(ppp, i=which(ppp[[col]]==6), j=col, value=NA)
		#for (index in (1:LXn)){
		#message(index)
		#set(RM, i=which(RM[['a']]==index), j='a', value= (index+diffL) )
		#set(RM, i=which(RM[['b']]==index), j='b', value= (index+diffL) )
		#}


		################################################################################
		# (5.0) Creating  and Changing to PlotPath
		plot_path <- paste(outcomes_plot_path, feature_plot_path,sep="")
